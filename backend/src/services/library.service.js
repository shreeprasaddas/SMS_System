...
module.exports = exports;
    ...
    const bookIssue = await BookIssue.findOne({ _id: issueId, schoolId })
      .populate('book')
      .populate('member');

    if (!bookIssue) throw new AppError('Book issue record not found', 404);
    if (bookIssue.status === 'RETURNED') throw new AppError('Book already returned', 400);
    ...
    const returnNumber = `RETURN-${Date.now()}`;

    const bookReturn = new BookReturn({
      schoolId,
      bookIssue: issueId,
      book: bookIssue.book._id,
      member: bookIssue.member._id,
      returnNumber,
      returnDate,
    ...
    bookIssue.status = bookCondition === 'LOST' ? 'LOST' : bookCondition === 'DAMAGED' ? 'DAMAGED' : 'RETURNED';
    bookIssue.returnDate = returnDate;
    bookIssue.actualReturnDate = returnDate;
    bookIssue.fineAmount = fineAmount;

    // Update book and member counts
    const book = await Book.findById(bookIssue.book._id);
    const member = await LibraryMember.findById(bookIssue.member._id);
    ...
    return bookReturn.populate(['bookIssue', 'book', 'member', 'receivedBy']);
  }

  /**
   * Get book issues
   */
  static async getBookIssues(schoolId, filters = {}) {
    const { page = 1, limit = 10, member, book, status, overdueOnly } = filters;
    ...
    const [data, total] = await Promise.all([
      BookIssue.find(query)
        ...
        .populate('member', 'membershipNumber member')
   ...
   * Get library statistics
   */
  static async getLibraryStats(schoolId) {
    const [
      totalBooks,
      availableBooks,
      totalMembers,
      ...
      Book.countDocuments({ schoolId, availableCopies: { $gt: 0 } }),
      LibraryMember.countDocuments({ schoolId }),
      ...
      LibraryFine.countDocuments({ schoolId, status: 'PENDING' }),
    ]);
